
resource "azurerm_public_ip" "publicIPworker" {
  name                         = "publicIPworker"
  location                     = "${var.region}"
  resource_group_name          = "${azurerm_resource_group.dev-group.name}"
  public_ip_address_allocation = "dynamic"
}

resource "azurerm_network_security_group" "worker-sg" {
  name                = "worker-sg"
  location            = "${var.region}"
  resource_group_name = "${azurerm_resource_group.dev-group.name}"

  security_rule {
    name                       = "SSH"
    priority                   = 1001
    direction                  = "Inbound"
    access                     = "Allow"
    protocol                   = "Tcp"
    source_port_range          = "*"
    destination_port_range     = "22"
    source_address_prefix      = "*"
    destination_address_prefix = "*"
  }
}

resource "azurerm_network_interface" "nic-worker" {
  count               = 1
  name                = "nic-worker"
  location            = "${var.region}"
  resource_group_name = "${azurerm_resource_group.dev-group.name}"

  ip_configuration {
    name                          = "nic-worker"
    subnet_id                     = "${azurerm_subnet.subnet1.id}"
    private_ip_address_allocation = "dynamic"
    public_ip_address_id          = "${azurerm_public_ip.publicIPworker.id}"
  }
}

resource "random_id" "randomId-worker" {
  keepers = {
    # Generate a new ID only when a new resource group is defined
    resource_group = "${azurerm_resource_group.dev-group.name}"
  }

  byte_length = 8
}

resource "azurerm_storage_account" "storageaccount-worker" {
  name                = "diag${random_id.randomId-etcd.hex}"
  resource_group_name = "${azurerm_resource_group.dev-group.name}"
  location            = "${var.region}"
  account_replication_type = "LRS"
  account_tier = "Standard"
}

resource "azurerm_virtual_machine" "worker-vm" {
  name = "worker-vm"
  location = "${var.region}"
  resource_group_name = "${azurerm_resource_group.dev-group.name}"
  network_interface_ids = [
    "${azurerm_network_interface.nic-worker.id}"]
  vm_size = "Standard_B1s"
  delete_os_disk_on_termination = true

  storage_os_disk {
    name = "OsDisk-worker"
    caching = "ReadWrite"
    create_option = "FromImage"
    managed_disk_type = "Premium_LRS"
  }


  storage_image_reference {
    publisher = "RedHat"
    offer = "RHEL"
    sku = "7.3"
    version = "latest"
  }

  os_profile {
    computer_name = "worker"
    admin_username = "azureuser"
  }

  os_profile_linux_config {
    disable_password_authentication = true
    ssh_keys {
      path = "/home/azureuser/.ssh/authorized_keys"
      key_data = "ssh-rsa  sursingh10@WKMGB0869902"
    }
  }

  boot_diagnostics {
    enabled = "true"
    storage_uri = "${azurerm_storage_account.storageaccount-worker.primary_blob_endpoint}"
  }
}